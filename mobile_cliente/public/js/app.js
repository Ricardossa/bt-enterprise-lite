/**
 * BT CLIENT PRO - APP ENGINE v2.3.1 (Diamond Native Fixed)
 * Aplicação Nativa do Cliente Final - 100% Sincronizada
 */
...
window.App = {
    ...
    async init() {
        // [v2.3.0] Detecta a base da URL dinamicamente e limpa sufixos
        const savedUrl = localStorage.getItem('bt_manual_url');
        if (savedUrl) {
            try {
                const parsed = new URL(savedUrl.startsWith('http') ? savedUrl : 'https://' + savedUrl);
        ...
        if (!this.clientUuid) {
            this.log("Aguardando identificação...");
            window.location.href = 'fidelidade/index.php?redirect=app';
            return;
        }
    ...
    async loadProfile() {
        try {
            const url = `${this.baseUrl}api/v1/cliente.php?uuid=${this.clientUuid}`;
        ...
        } catch (e) { this.log("Erro ao carregar perfil: " + e.message); }
    },
